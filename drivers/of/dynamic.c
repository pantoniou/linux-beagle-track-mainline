...
int of_reconfig_get_state_change(unsigned long action, void *arg)
{
	...
	case OF_RECONFIG_ATTACH_NODE:
		prev_state = 0;
		new_state = status_state != 0;	/* -1 & 0 status either missing or okay */
		break;
	case OF_RECONFIG_DETACH_NODE:
		prev_state = status_state != 0;	/* -1 & 0 status either missing or okay */
		new_state = 0;
		break;
	case OF_RECONFIG_ADD_PROPERTY:
		if (is_status) {
			prev_state = 1;	/* no status property -> enabled (legacy) */
	...
	case OF_RECONFIG_REMOVE_PROPERTY:
		if (is_status) {
			...
			new_state = 1;	/* no status property -> enabled (legacy) */
		}
		break;
	case OF_RECONFIG_UPDATE_PROPERTY:
		if (is_status) {
...
static int __of_changeset_entry_notify(struct of_changeset_entry *ce, bool revert)
{
...
int of_changeset_revert(struct of_changeset *ocs)
{
	...
	mutex_unlock(&of_mutex);
	list_for_each_entry_reverse(ce, &ocs->entries, node) {
		ret = __of_changeset_entry_notify(ce, 1);
		if (ret) {
			list_for_each_entry_continue(ce, &ocs->entries, node)
