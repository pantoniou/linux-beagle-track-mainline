		...
		struct device_node *target, struct property *prop)
{
	...
	if (tprop != NULL) {
		if (propn != NULL)
			return of_changeset_update_property(&ov->cset,
					target, propn, tprop);
		else
			return of_changeset_remove_property(&ov->cset,
					target, tprop);
	}
		...
		struct device_node *target, struct device_node *child)
{
	...
	if (tchild != NULL) {
		...
		if (!remove)
			...
			ret = of_changeset_detach_node(&ov->cset, tchild);
...
static int overlay_is_topmost(struct of_overlay *ov, struct device_node *dn)
{
	...
	list_for_each_entry_reverse(ovt, &ov_list, node) {
		...
		list_for_each_entry(ce, &ovt->cset.te_list, node) {
...
static int overlay_removal_is_ok(struct of_overlay *ov)
{
	...
	list_for_each_entry(ce, &ov->cset.te_list, node) {
