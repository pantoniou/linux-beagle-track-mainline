		...
		struct of_overlay_device_entry *de, int revert)
{
	...
	if (adap) {
		pr_debug("%s: removing adap %s\n", __func__,
				dev_name(&adap->dev));
		put_device(&adap->dev);
	}

	if (parent_pdev) {
		pr_debug("%s: removing parent_pdev %s\n", __func__,
				dev_name(&parent_pdev->dev));
 ...
 * "target-path" property containing the full node path of the target
 * "target-alias" property containing the alias name of the target
 *
 */
struct device_node *find_target_node(struct device_node *info_node)
{
	...
	/* last attempt, alias */
	ret = of_property_read_string(info_node, "target-alias", &alias);
	if (ret == 0 && of_aliases != NULL) {
		ret = of_property_read_string(of_aliases, alias, &path);
		if (ret == 0)
			return of_find_node_by_path(path);
	}
