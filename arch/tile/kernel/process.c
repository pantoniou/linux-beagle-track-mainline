...
void prepare_exit_to_usermode(struct pt_regs *regs, u32 thread_info_flags)
{
	...
	do {
		/*
		 * schedule() may switch from a task in an irq that has
		 * not called user_exit(), to a task that did call
		 * user_exit() from a syscall or exception, so we need
		 * to make sure we call user_exit() again here.
		 */
		user_exit();
